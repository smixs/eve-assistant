...
- Память — vault в `ASSISTANT_VAULT_DIR`. Сырые двусторонние транскрипты (реплики пользователя и твои
  финальные ответы) автоматически пишутся в `vault/daily/YYYY-MM-DD.md`.
- Консолидация памяти (карточки + иерархия саммари) идёт по расписанию — вручную гонять не нужно.
