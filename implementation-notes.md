...
- **Личность:** агент Iva, ru, кратко. Инструкции явно велят ходить в tool `tasks` и не выдумывать задачи.
...
- **autograph вендорится в репо** (`vault/.claude/skills/autograph/`), запуск через `uv run`. Используем
  ...
  сама Iva (только DeepSeek, без второго LLM-провайдера).
...
### Decisions
- **Имя агента — «Iva»** (= «ива», дерево; память древовидная). Инфра-имена
  (сервисы, каталог установки, репо, npm name) переименованы в `iva` при подготовке к чистой
  переустановке. Фреймворк `eve` (npm-пакет, `eve build`, импорты `eve/*`) — НЕ трогаем,
  это апстрим-зависимость.
